...
import { fetchCrimeIndividualSampleThunk } from '../../../store/crime';


function CrimeIndividual() {
  const { id } = useParams();
  ...
  // tons of use selectors.  There has to be a better way.
...
console.log('i am what i want ',grandTotalIncarceration, grandTotalProbation)
  ...
  useEffect(() => {
      ...
      dispatch(fetchCrimeIndividualSampleThunk(id)).then(() => setLoaded(true));
        }, [id, dispatch]);
  ...
  return (
    <div>
      ...
      <h1 className="crimeLandingHeader">The Rapsheet for: {crimeName}</h1>
      ...
      export default CrimeIndividual